  ...
  return (
    <div style={{ padding: '16px', maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        <div>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: '#e8e0d0', margin: 0 }}>System Audit History</h1>
          <p style={{ fontSize: 11, color: '#5a6a40', margin: '3px 0 0' }}>Complete audit trail of all system actions</p>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {[['Screenings',counts.screenings,'#68c87a'],['Reviews',counts.reviews,'#cc9944'],['Uploads',counts.uploads,'#7899cc']].map(([l,v,c]) => (
            <div key={String(l)} style={{ padding: '5px 10px', background: `${c}12`, border: `1px solid ${c}30`, borderRadius: 6, fontSize: 11, color: String(c), display: 'flex', gap: 5 }}>
              {l}: <strong>{v}</strong>
            </div>
          ))}
        </div>
      ...
      <div className="card-1" style={{ borderRadius: 8, padding: '12px 14px', marginBottom: 14 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative', minWidth: 160, flex: '1 1 180px' }}>
            ...
            <input className="admin-input" placeholder="Search user, action…" value={search} onChange={e=>setSearch(e.target.value)}
              style={{ width: '100%', padding: '6px 10px 6px 26px', fontSize: 12 }}/>
          </div>
          <select className="admin-input" value={statusFilter} onChange={e=>setStatusFilter(e.target.value)} style={{ padding: '6px 10px', fontSize: 12, cursor: 'pointer', minWidth: 130, flex: '1 1 140px' }}>
            {STATUSES.map(s => <option key={s} value={s} style={{ background: '#1d2113' }}>{s}</option>)}
          </select>
          {(search||statusFilter!=='All') && (
            <button className="btn-ghost" style={{ padding: '6px 10px', borderRadius: 5, fontSize: 11, cursor: 'pointer' }} onClick={() => { setSearch(''); setStatusFilter('All'); }}>
              Clear
            </button>
          )}
          <div style={{ marginLeft: 'auto', fontSize: 11, color: '#4a5a30' }}>{filtered.length} record{filtered.length!==1?'s':''}</div>
        </div>
      </div>
      ...
      <div className="card-1" style={{ borderRadius: 8, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '10px 16px', borderTop: '1px solid rgba(74,90,42,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(29,33,19,0.8)', flexWrap: 'wrap', gap: 8 }}>
